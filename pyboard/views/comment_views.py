...
from pyboard.models import Question, Comment, Answer
...
def create_question(question_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        comment = Comment(user=g.user,content=form.content.data,
                          create_date=datetime.now(),question = q)
...
def create_answer(answer_id):
    ...
    if request.method == 'POST' and form.validate_on_submit():
        comment = Comment(user=g.user, content=form.content.data,create_date = datetime.now(), answer=a)
        db.session.add(comment)
        db.session.commit()
